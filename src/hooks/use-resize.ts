...
function distributePixelsProportionally(
  ...
  const finalChanges = [...constrainedChanges];
...
export function useResize() {
  ...
  const prevColumnSizes = useRef<ColumnSizingState>({});
  ...
  const updateColumnConstraints = useCallback((table: Table<unknown>) => {
  ...
  const initializeColumnSizing = useCallback(() => {
  ...
  const handleColumnSizing = useCallback(
    (
      ...
      const pixelDelta = Math.round(columnSizingInfo.deltaOffset);
      const updatedSizing = { ...prevColumnSizes.current };
